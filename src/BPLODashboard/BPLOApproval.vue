  ...
  <section class="approval-surface">
    ...
    <div class="toolbar">
      ...
      <select v-model="filter" class="control">
        <option value="READY">Ready for BPLO review</option>
        ...
        <tbody>
          <tr v-for="app in paginated" :key="app.id">
            ...
            <td class="row-actions">
              <button class="btn ghost" type="button" @click="selected = app">View</button>
              <button class="btn primary" type="button" :disabled="!canDecide(app)" @click="approve(app)">Approve</button>
              <button class="btn danger" type="button" :disabled="!canDecide(app)" @click="openReject(app)">Reject</button>
            </td>
          </tr>
    ...
    <div v-if="rejecting" class="modal-backdrop" @click.self="rejecting = null">
      <form class="modal" @submit.prevent="reject">
        <header><h3>Reject final approval</h3><button class="x" type="button" @click="rejecting = null">x</button></header>
...
import { approveByBPLO, getApplications, rejectByBPLO } from '../services/applicationService'
...
const filtered = computed(() => {
  ...
  return applications.value.filter((app) => {
    const ready = app.marketApprovalStatus === 'APPROVED' && (app.bploStatus || 'PENDING') === 'PENDING'
    const statusMatch = filter.value === 'ALL' || (filter.value === 'READY' ? ready : (app.bploStatus || 'PENDING') === filter.value)
...
function canDecide(app) {
  return app.marketApprovalStatus === 'APPROVED' && (app.bploStatus || 'PENDING') === 'PENDING'
}
...
async function approve(app) {
  try {
    ...
    showToast(error.message || 'BPLO approval failed', 'error')
  }
}
...
async function reject() {
  try {
    ...
    showToast(error.message || 'BPLO rejection failed', 'error')
  }
}
...
<style scoped>
.approval-surface{display:grid;gap:16px}.toolbar{display:flex;gap:10px;flex-wrap:wrap}.control{height:42px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;padding:0 12px;min-width:220px}.table-card{background:#fff;border:1px solid #e2e8f0;border-radius:8px;overflow:auto;box-shadow:0 14px 34px rgba(15,23,42,.06)}.approval-table{width:100%;border-collapse:collapse;min-width:840px}.approval-table th,.approval-table td{padding:14px 16px;border-bottom:1px solid #edf2f7;text-align:left}.approval-table th{font-size:12px;text-transform:uppercase;color:#64748b;background:#f8fafc}.approval-table small{display:block;color:#64748b;margin-top:4px}.row-actions{display:flex;gap:8px;justify-content:flex-end}.btn{border:0;border-radius:8px;padding:9px 12px;font-weight:800;cursor:pointer}.btn:disabled{opacity:.45;cursor:not-allowed}.primary{background:#059669;color:#fff}.danger{background:#e11d48;color:#fff}.ghost{background:#f1f5f9;color:#334155}.badge{display:inline-flex;min-width:88px;justify-content:center;border-radius:999px;padding:5px 9px;font-size:12px;font-weight:800}.pending{background:#fef3c7;color:#92400e}.approved,.endorsed{background:#dcfce7;color:#166534}.rejected{background:#fee2e2;color:#991b1b}.pagination{display:flex;align-items:center;justify-content:flex-end;gap:12px}.loading,.empty{padding:28px;text-align:center;color:#64748b}.spinner{display:inline-block;width:18px;height:18px;border:3px solid #bbf7d0;border-top-color:#059669;border-radius:50%;animation:spin .8s linear infinite;margin-right:8px;vertical-align:middle}.toast{position:fixed;right:22px;top:18px;z-index:50;padding:12px 14px;border-radius:8px;font-weight:800;box-shadow:0 18px 45px rgba(15,23,42,.16)}.toast.success{background:#dcfce7;color:#166534}.toast.error{background:#fee2e2;color:#991b1b}.modal-backdrop{position:fixed;inset:0;background:rgba(15,23,42,.45);display:grid;place-items:center;z-index:40;padding:20px}.modal{width:min(620px,100%);background:#fff;border-radius:8px;padding:18px}.modal header{display:flex;align-items:center;justify-content:space-between}.x{border:0;background:#f1f5f9;border-radius:8px;width:32px;height:32px}.modal dl{display:grid;gap:12px}.modal dt{font-weight:800;color:#334155}.modal dd{margin:4px 0 0;color:#475569}.remarks{width:100%;border:1px solid #cbd5e1;border-radius:8px;padding:10px;resize:vertical}.modal-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:14px}@keyframes spin{to{transform:rotate(360deg)}}@media (max-width:760px){.toolbar{display:grid}.control{min-width:0;width:100%}.pagination{justify-content:center}}
</style>
