...
<template>
  <aside class="treasurer-sidebar" :class="{ collapsed }" aria-label="BPLO sidebar">
...
const items = [
  ...
  { id: 'review', label: 'Review Applications', to: '/bplo', routeName: 'BPLO' },
...
function toggle() {
  ...
  try { localStorage.setItem('bploMenuCollapsed', String(collapsed.value)) } catch (e) {}
  try {
...
onMounted(() => {
  try {
    const stored = localStorage.getItem('bploMenuCollapsed')
    if (props.forceOpen) {
